...
#define BTN_BINARY 22   // GP22 → binary dump (ACTIVE)
...
int main(void) {
    ...
    gpio_init(BTN_BINARY);
    gpio_pull_up(BTN_BINARY);

    bool prev_rotate = true;
    bool prev_dump   = true;
    bool prev_binary = true;
    ...
    while (true) {
        ...
        if (c == 'D' || c == 'd') dump_active_compact();
            ...
            // make sure any pending text is out
            stdio_flush();
        ...
        // ---- Check Button C (Binary Dump) ----
        bool now_binary = gpio_get(BTN_BINARY);
        if (!now_binary && prev_binary) {   // pressed (LOW edge)
            stdio_flush();
            dump_active_compact_bit();
            stdio_flush();
        }

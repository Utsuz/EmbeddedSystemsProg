...
// Saving obtained time 
static uint32_t ntp_epoch_ref = 0;
static uint64_t local_us_ref = 0;
static bool ntp_synced_once = false;
...
void main_task(__unused void *params) {
    ...
    while(true) {
        ...
        if (amount == NTP_MSG_LEN && mode == 0x4 && stratum != 0){
            uint32_t seconds_since_1900 = (request[40] << 24) | (request[41] << 16) | (request[42] << 8) | request[43];
            uint32_t seconds_since_1970 = seconds_since_1900 - NTP_DELTA;
            time_t epoch = seconds_since_1970;
            
            // Add Singapore offset (+8h)
            epoch += 8 * 3600;

            // Print actual NTP time
            struct tm *utc = gmtime(&epoch);
            printf("Time (NTP): %02d/%02d/%04d %02d:%02d:%02d\n", utc->tm_mday, 
                   utc->tm_mon + 1, utc->tm_year + 1900, utc->tm_hour, 
                   utc->tm_min, utc->tm_sec);
            
            // Store baseline and compare difference
            if (!ntp_synced_once) {
                ntp_epoch_ref = epoch;
                local_us_ref = time_us_64();
                ntp_synced_once = true;
                printf("[sync] Initial NTP baseline set.\n");
            } else {
                uint64_t now_us = time_us_64();
                double elapsed = (now_us - local_us_ref) / 1000000.0;
                double calc_epoch = ntp_epoch_ref + elapsed;
                double diff = (double)epoch - calc_epoch;

                // Convert calc_epoch to human time for display
                time_t calc_time = (time_t)calc_epoch;
                struct tm *calc_tm = gmtime(&calc_time);

                printf("Time (Calc): %02d/%02d/%04d %02d:%02d:%02d | Δ=%+.2f s\n", 
                    calc_tm->tm_mday, calc_tm->tm_mon + 1, calc_tm->tm_year + 1900,
                    calc_tm->tm_hour, calc_tm->tm_min, calc_tm->tm_sec, diff);

                // If drift > 1s, reset baseline
                if (fabs(diff) > 1.0) {
                ntp_epoch_ref = epoch;
                local_us_ref = time_us_64();
                printf("[sync] Large drift (%.2fs) → baseline reset.\n", diff);
              }
            }
        }
